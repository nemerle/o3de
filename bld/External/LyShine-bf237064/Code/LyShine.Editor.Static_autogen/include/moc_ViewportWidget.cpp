/****************************************************************************
** Meta object code from reading C++ file 'ViewportWidget.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.15.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include <memory>
#include "../../../../../../Gems/LyShine/Code/Editor/ViewportWidget.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'ViewportWidget.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.15.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_ViewportWidget_t {
    QByteArrayData data[7];
    char stringdata0[99];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_ViewportWidget_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_ViewportWidget_t qt_meta_stringdata_ViewportWidget = {
    {
QT_MOC_LITERAL(0, 0, 14), // "ViewportWidget"
QT_MOC_LITERAL(1, 15, 20), // "UserSelectionChanged"
QT_MOC_LITERAL(2, 36, 0), // ""
QT_MOC_LITERAL(3, 37, 24), // "HierarchyItemRawPtrList*"
QT_MOC_LITERAL(4, 62, 5), // "items"
QT_MOC_LITERAL(5, 68, 18), // "EnableCanvasRender"
QT_MOC_LITERAL(6, 87, 11) // "RefreshTick"

    },
    "ViewportWidget\0UserSelectionChanged\0"
    "\0HierarchyItemRawPtrList*\0items\0"
    "EnableCanvasRender\0RefreshTick"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_ViewportWidget[] = {

 // content:
       8,       // revision
       0,       // classname
       0,    0, // classinfo
       3,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: name, argc, parameters, tag, flags
       1,    1,   29,    2, 0x08 /* Private */,
       5,    0,   32,    2, 0x08 /* Private */,
       6,    0,   33,    2, 0x08 /* Private */,

 // slots: parameters
    QMetaType::Void, 0x80000000 | 3,    4,
    QMetaType::Void,
    QMetaType::Void,

       0        // eod
};

void ViewportWidget::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        auto *_t = static_cast<ViewportWidget *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->UserSelectionChanged((*reinterpret_cast< HierarchyItemRawPtrList*(*)>(_a[1]))); break;
        case 1: _t->EnableCanvasRender(); break;
        case 2: _t->RefreshTick(); break;
        default: ;
        }
    }
}

QT_INIT_METAOBJECT const QMetaObject ViewportWidget::staticMetaObject = { {
    QMetaObject::SuperData::link<AtomToolsFramework::RenderViewportWidget::staticMetaObject>(),
    qt_meta_stringdata_ViewportWidget.data,
    qt_meta_data_ViewportWidget,
    qt_static_metacall,
    nullptr,
    nullptr
} };


const QMetaObject *ViewportWidget::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *ViewportWidget::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_ViewportWidget.stringdata0))
        return static_cast<void*>(this);
    return AtomToolsFramework::RenderViewportWidget::qt_metacast(_clname);
}

int ViewportWidget::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = AtomToolsFramework::RenderViewportWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 3)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 3;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 3)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 3;
    }
    return _id;
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
