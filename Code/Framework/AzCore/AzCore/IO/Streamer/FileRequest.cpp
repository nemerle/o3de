...
namespace AZ::IO
{
        ...
        AZStd::chrono::system_clock::time_point deadline, IStreamerTypes::Priority priority)
    {
        ...
        m_command.emplace<RequestCommands::ReadRequestData>(AZStd::move(path), output, outputSize, offset, size, deadline, priority);
    }
        ...
        AZStd::chrono::system_clock::time_point deadline, IStreamerTypes::Priority priority)
    {
        AZ_Assert(AZStd::holds_alternative<AZStd::monostate>(m_command),
            "Attempting to set FileRequest to 'ReadRequest', but another task was already assigned.");
        m_command.emplace<RequestCommands::ReadRequestData>(AZStd::move(path), allocator, offset, size, deadline, priority);
    }
        ...
        u64 offset, u64 size, bool sharedRead)
    {
        ...
        m_command.emplace<RequestCommands::ReadData>(output, outputSize, AZStd::move(path), offset, size, sharedRead);
        SetOptionalParent(parent);
    }
    ...
    void FileRequest::CreateDedicatedCacheCreation(RequestPath path, const FileRange& range, FileRequest* parent)
    {
        ...
        m_command.emplace<RequestCommands::CreateDedicatedCacheData>(AZStd::move(path), range);
        SetOptionalParent(parent);
    }
    ...
    void FileRequest::CreateDedicatedCacheDestruction(RequestPath path, const FileRange& range, FileRequest* parent)
    {
        ...
        m_command.emplace<RequestCommands::DestroyDedicatedCacheData>(AZStd::move(path), range);
        SetOptionalParent(parent);
    }

    void FileRequest::CreateReport(FileRequestReportType reportType)
    {
        AZ_Assert(AZStd::holds_alternative<AZStd::monostate>(m_command),
            "Attempting to set FileRequest to 'Report', but another task was already assigned.");
        m_command.emplace<RequestCommands::ReportData>(reportType);
    }
